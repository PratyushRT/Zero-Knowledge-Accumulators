...
#include <gmp.h>
#include <string.h>
//gcc -o update_mem_acc update_mem_acc.c -I ~/.local/include/pbc -L ~/.local/lib -Wl,-rpath ~/.local/lib  -l pbc -lgmp -lm -Wsystem-headers
...
int main(void){
    ...
    FILE *update;
    char param5[102400];
    update = fopen("zkacc/test/update_mem.txt","r");
    ...
    element_mul(pow, pow, acc_upd);
    element_pow_zn(sigma, sigma, acc_upd);
